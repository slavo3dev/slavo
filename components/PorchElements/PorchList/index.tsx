...
export const PorchList: React.FC<PorchListProps> = ({
  ...
  const filteringUpdatesPerUser = useMemo(() => {
    const updates = filtered
      ? dailyUpdates.filter((porch) => porch.email === userEmail)
      : dailyUpdates;
    return updates
      .slice()
      .sort(
  ...
  }, [dailyUpdates, userEmail, filtered]);
    ...
    const fetchLearningDays = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/porch/user/count?email=${userEmail}`,
        );

        if (!res.ok) {
          const text = await res.text(); // prevents JSON crash
          throw new Error(text || "Failed to fetch learning days");
        }

        const data = await res.json(); // ✅ now safe
        setLearningDays(data.learningDays ?? 0);
      } catch (error) {
        console.error("Failed to fetch learning days:", error);
      }
    };
