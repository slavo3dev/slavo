...
export const OpenAI: FC = () => {
  ...
  const handleApiAI = async () => {
    ...
    try {
      ...
      setErrMsg(
        "We're sorry, we're currently experiencing some difficulties retrieving data.\n Please come back later or try again. \n Thank you for your patience.",
      );
      setIsLoading(false);
    }
  };

  const resposeAI = errMsg ? (
    <div className="text-gray-700 text-base p-8">{errMsg}</div>
  ) : (
        ...
        <div className="bg-white shadow-inner rounded px-8 pt-6 pb-8 w-full lg:max-w-full">
          <div className="mb-7 pl-8 pr-8 flex pb-4 flex-col">
            <label className="block text-gray-700 first-letter:text-7xl first-letter:font-normal first-letter:text-blue-600 lg:text-3xl md:text-xl font-bold mb-2 lg:text-left md:text-center sm:text-xs">
              Ask Question About:
              <p className="text-gray-700 lg:text-xl font-normal mt-7">
                HTML | CSS | JavaScript | TypeScript | React | NodeJS
                | API
              </p>
              <p className="text-gray-700 lg:text-xl font-normal mb-7">
          ...
          {isLoading ? (
            <Loader title={"Please Wait... Answer is coming...."} />
          ) : (
            resposeAI
          )}
        </div>
