...
export const CreateAccount: FC<CreateAccountProps> = ( { signIn }  ) =>
{
	...
	const signUpWithEmail = async () => {
		try {
			...
			if (error) {
				...
				let customMessage;
				if (error.message === 'Email address "zach@gmail.com" cannot be used as it is not authorized') {
					customMessage = "The email address is invalid. Please provide a valid email address to continue.";
				} else if (error.message === "Passwords Not Matching") {
					customMessage = "Passwords do not match.";
				} else {
					customMessage = "Cannot create an account without providing the information above.";
				}
	...
	// !!! 
	// need to setTimeout when displaying Congrats Message 
