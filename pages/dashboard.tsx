...
import { HeadBasePage, Calendar } from "@components";
...
const Dashboard: NextPage = () => {
  ...
  const fetchUserAndLearningData = async () => {
    ...
    try {
      const response = await fetch(
        `/api/getUserActivity?email=${encodeURIComponent(userInfo.email)}`,
      );
      const { userActivityData, learningData } =
        await response.json();
  ...
  const calculateStreaks = (data: { created_at: string }[]) => {
    ...
    sorted.forEach((date) => {
      ...
      const diff = Math.floor(
        (date.getTime() - last.getTime()) / (1000 * 3600 * 24),
      );
    ...
    const diffToToday = last
      ? Math.floor(
          (today.getTime() - (last as Date).getTime()) /
            (1000 * 3600 * 24),
        )
      : 0;
  ...
  const calculateWeeklyLearningDays = (
    ...
    try {
      ...
      if (userActivityData) {
        ...
        await supabase
          .from("user_activity")
          .insert([
            { user_email: userInfo.email, weekly_goal: selectedGoal },
          ]);
      }
  ...
  const memoizedLearningDates = useMemo(
    () => learningDates,
    [learningDates],
  );
  ...
  return (
    <>
      <HeadBasePage
        title="Dashboard | Slavo"
        description="Your habit and learning dashboard."
        noIndex
      />
