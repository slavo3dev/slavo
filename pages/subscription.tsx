...
import { HeadBasePage, LoginModal } from "@components";
...
const SubscriptionPage: NextPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubscription = async () => {
      const { data, error } = await supabase
        .from("profile")
        .select(
          "is_subscribed, interval, cancel_at, cancel_at_period_end",
        )
        .eq("id", userInfo.id)
        .single();
    ...
    return (
      <div className="max-w-2xl py-10 mx-auto text-center text-blue-500">
        ...
        <p className="text-gray-600">
          We couldn’t find your subscription info.
        </p>
      </div>
    );
  }
  ...
  const handleCancelSubscription = async () => {
    ...
    if (response.ok) {
      ...
      if (
        ...
        subscriptionData.interval !== "unlimited"
      ) {
  ...
  return (
    <>
      <HeadBasePage
        title="My Subscription | Slavo"
        description="View and manage your Slavo subscription details."
      />
