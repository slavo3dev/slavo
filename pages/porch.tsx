...
function FactList ( { porchs, setPorchs }: any ) {
    ...
    const [dailyUpdates, setDailyUpdates] = useState(porchs);
    const [buttonTitle, setButtonTitle] = useState("Track Your Daily Updates");
    ...
    const filteringUpdatesPerUser = dailyUpdates.filter( ( porch: { email: string; } ) => porch.email === userInfo?.email );
                ...
                <div className="px-4 py-5 sm:p-6">
                    <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-500">Growth and Learning News</p>
                          { userInfo?.email ?
                            <>
                              <p className="mt-5 text-lg font-medium text-gray-800">You've been dedicated to learning for <b>{ learningDays }</b> days!</p>
                              <button onClick={handleFiltering} className="mt-3 px-4 py-2 text-sm font-medium text-white bg-blue-500    rounded-lg shadow hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-300 ease-in-out">
                                 {buttonTitle}
                              </button>
                            </>
                              : null }
